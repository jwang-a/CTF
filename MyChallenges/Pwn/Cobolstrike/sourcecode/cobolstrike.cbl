      ...
      *FD SYSIN.
       ...
       01 FILES.
           ...
           05 slashCnt PIC 9(3).
           05 fidx PIC 9(2).
           05 foff PIC S9(10) VALUE ZERO BINARY.
      ...
      *         END-IF
               ...
               IF opt IS EQUAL TO 1 THEN
                   ...
                   DISPLAY "Index: "
                   ACCEPT fidx
                   MOVE ZERO to slashCnt
                   INSPECT fnm TALLYING slashCnt FOR ALL "/"
                   IF (slashCnt IS >= 1) THEN
                   ...
                   ELSE
                       IF (fidx IS >= 1) AND (fidx IS <= 16) THEN
                           IF fsz(fidx) EQUAL TO ZERO THEN
                               ...
                               CALL "malloc"
                               USING BY VALUE fsz(fidx)
                               RETURNING fptr(fidx)

                               IF fptr(fidx) NOT EQUAL TO NULL THEN
                                   CALL "CBL_OPEN_FILE"
                                       USING fnm 1 3 0 ffd(fidx)
                                   IF RETURN-CODE NOT EQUAL TO 0 THEN
                                       DISPLAY "failed to open file"
                                       CALL "free"
                                       USING BY VALUE fptr(fidx)
                                       SET ffd(fidx) TO 0
                                       SET fsz(fidx) TO 0
                                       SET fptr(fidx) TO NULL
                                   END-IF
