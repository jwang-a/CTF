  ...
  while (1){
    ...
    switch(req->data.nr){
...
#endif
        //CREDITS: Special thanks to NeSE for pointing out that req->data.args[0] is not properly cast to pid_t
        //Code fixed, but binary provided in distribute not recompiled, make and replace to use patched version
        if(((pid_t)req->data.args[0])==selfPid || ((pid_t)req->data.args[0])==reaperTid){
      ...
      case __NR_openat2:
        ...
        //CREDITS: Special thanks to 0rganizers for pointing out impl flaw in handling relative paths for open
        //Code fixed, but binary provided in distribute not recompiled, make and replace to use patched version
        int dirFd;
        ...
        if((req->data.nr==__NR_open && (tmpFd = openat(dirFd,fname,req->data.args[1],req->data.args[2]))==-1) ||
           (req->data.nr==__NR_openat && (tmpFd = openat(dirFd,fname,req->data.args[2],req->data.args[3]))==-1) ||
           (req->data.nr==__NR_openat2 && (tmpFd = syscall(__NR_openat2,dirFd,fname,openHow,req->data.args[3]))==-1)){
          close(dirFd);
          if(req->data.nr==__NR_openat2)
            free(openHow);
...
#endif
          close(tmpFd);
          if((req->data.nr==__NR_open && (tmpFd = openat(AT_FDCWD,FAKEFLAGPATH,req->data.args[1],req->data.args[2]))==-1) ||
      ...
      case __NR_open_by_handle_at:
        ...
        if((tmpFd = open_by_handle_at(dirFd,fHandle,req->data.args[2]))==-1){
          close(dirFd);
          free(fHandle);
          SETSECCOMPRESP(resp,-errno,-1,0);
          break;
...
#endif
          close(tmpFd);
          if((tmpFd = openat(AT_FDCWD,FAKEFLAGPATH,req->data.args[2]))==-1){
            SETSECCOMPRESP(resp,-errno,-1,0);
            break;
          }
        }
