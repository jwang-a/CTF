...
MEMLIST *handlerFetchMemList(HANDLERMEMSTATE *memState,int guestProtRequired,void *addr){
  ...
  /* This &/!= originally didn't have parentheses, resulting in an unintended bug, source patched, but binary not fixed
   * recompile yourself if you feel the need to do so */
